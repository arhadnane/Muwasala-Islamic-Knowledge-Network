...
namespace AbuDawudImporter;

/// <summary>
/// Importer dédié pour Sunan Abi Dawud depuis sunnah.com
/// </summary>
public class Program
{
    ...
    {
        Console.WriteLine("🕌 SUNAN ABI DAWUD DIRECT IMPORT");
        Console.WriteLine("=================================");
        ...
        try
        {
            ...
            Console.WriteLine("📚 Starting Sunan Abi Dawud import...");
            
            var hadiths = await ImportAbuDawudAsync();
            
            // Sauvegarder en JSON
            var outputPath = "../Tools/sunan_abudawud_complete.json";
    ...
    private static async Task<List<HadithRecord>> ImportAbuDawudAsync()
    {
        var allHadiths = new List<HadithRecord>();
        
        // Récupérer la page principale
        Console.WriteLine("🔗 Fetching: https://sunnah.com/abudawud");
        var mainHtml = await GetPageContentAsync("https://sunnah.com/abudawud");
        ...
        Console.WriteLine($"📖 Found {books.Count} books in Sunan Abi Dawud");
    ...
    private static List<BookInfo> ExtractBooks(string html)
    {
        ...
        var bookNodes = doc.DocumentNode.SelectNodes("//a[contains(@href, '/abudawud/')]");
        ...
        if (bookNodes != null)
        {
            foreach (var bookNode in bookNodes)
            {
                ...
                var match = Regex.Match(href, @"/abudawud/(\d+)");
    ...
    private static async Task<List<HadithRecord>> ImportBookAsync(BookInfo book)
    {
        ...
        var bookUrl = $"{_baseUrl}/abudawud/{book.Number}";
    ...
    private static string DetermineGrade(string gradeText)
    {
        ...
        // Pour Sunan Abi Dawud, par défaut Hasan (collection réputée fiable)
        return "Hasan";
    }
