...
foreach ($query in $testQueries) {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri "$searchEndpoint?query=$encodedQuery" -Method GET -TimeoutSec 60
        ...
        if ($response.webResults) {
            ...
            Write-Host "  📊 Response received (structure may vary)" -ForegroundColor Blue
        }
