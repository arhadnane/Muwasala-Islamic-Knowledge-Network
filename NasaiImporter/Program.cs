...
namespace NasaiImporter;

/// <summary>
/// Importer dédié pour Sunan an-Nasa'i depuis sunnah.com
/// </summary>
public class Program
{
    ...
    {
        Console.WriteLine("🕌 SUNAN AN-NASA'I DIRECT IMPORT");
        Console.WriteLine("=================================");
        ...
        try
        {
            ...
            Console.WriteLine("📚 Starting Sunan an-Nasa'i import...");
            
            var hadiths = await ImportNasaiAsync();
            
            // Sauvegarder en JSON
            var outputPath = "../Tools/sunan_nasai_complete.json";
    ...
    private static async Task<List<HadithRecord>> ImportNasaiAsync()
    {
        var allHadiths = new List<HadithRecord>();
        
        // Récupérer la page principale
        Console.WriteLine("🔗 Fetching: https://sunnah.com/nasai");
        var mainHtml = await GetPageContentAsync("https://sunnah.com/nasai");
        ...
        Console.WriteLine($"📖 Found {books.Count} books in Sunan an-Nasa'i");
    ...
    private static List<BookInfo> ExtractBooks(string html)
    {
        ...
        var bookNodes = doc.DocumentNode.SelectNodes("//a[contains(@href, '/nasai/')]");
        ...
        if (bookNodes != null)
        {
            foreach (var bookNode in bookNodes)
            {
                ...
                var match = Regex.Match(href, @"/nasai/(\d+)");
    ...
    private static async Task<List<HadithRecord>> ImportBookAsync(BookInfo book)
    {
        ...
        var bookUrl = $"{_baseUrl}/nasai/{book.Number}";
    ...
    private static string DetermineGrade(string gradeText)
    {
        ...
        // Pour Sunan an-Nasa'i, par défaut Hasan (collection très respectée)
        return "Hasan";
    }
